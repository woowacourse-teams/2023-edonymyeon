...
public class PostServiceSearchPostsTest {
    ...
    void 키워드로_검색했을때_내용이_제대로_검색되는지_확인한다() {
        // when
        var 검색결과 = postService.searchPosts(" 사회   초년 자취   거지 ", emptyFindingCondition).posts();
    ...
    void 키워드로_앞뒤_순서가_바뀌었을때_내용이_제대로_검색되는지_확인한다() {
        // when
        var 검색결과 = postService.searchPosts(" 거지 자취 사회  초년", emptyFindingCondition).posts();
    ...
    void 키워드로_동일한_키워드가_두개_포함되어도_내용이_제대로_검색되는지_확인한다() {
        // when
        var 검색결과 = postService.searchPosts(" 거지 자취 사회 거지 초년", emptyFindingCondition).posts();
