...
public class PostIntegrationTest extends IntegrationTest {
    ...
    void 로그인하지_않은_사용자가_게시글_하나를_상세조회하는_경우(@Autowired PostService postService) throws IOException {
        ...
        assertAll(
                ...
                () -> assertThat(response.body().jsonPath().getInt("reactionCountResponse.scrapCount")).isEqualTo(post.reactionCountResponse().scrapCount()),
    ...
    void 로그인한_사용자가_자신의_게시글_하나를_상세조회하는_경우(@Autowired PostService postService) throws IOException {
        ...
        final SpecificPostInfoResponse post = postService.findSpecificPost(postResponse.id(), new MemberIdDto(member.getId()));
    ...
    void 로그인한_사용자가_타인의_게시글_하나를_상세조회하는_경우(@Autowired PostService postService) throws IOException {
        ...
        final SpecificPostInfoResponse post = postService.findSpecificPost(postResponse.id(), new MemberIdDto(member2.getId()));
