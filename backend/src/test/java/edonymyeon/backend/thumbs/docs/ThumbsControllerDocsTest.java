...
public class ThumbsControllerDocsTest {
    ...
    void 회원_두명_가입하고_글쓰기_모킹(){
        ...
        when(memberRepository.findByEmailAndPassword(반응_하는_사람.getEmail(), 반응_하는_사람.getPassword())).thenReturn(Optional.of(반응_하는_사람));
    ...
    void 게시글에_좋아요를_한다() throws Exception {
        ...
        final MockHttpServletRequestBuilder 좋아요_요청 = put("/posts/" + 글.getId() + "/up")
    ...
    void 게시글에_싫어요를_한다() throws Exception {
        ...
        final MockHttpServletRequestBuilder 싫어요_요청 = put("/posts/" + 글.getId() + "/down")
                .header(HttpHeaders.AUTHORIZATION, "Basic "
                        + java.util.Base64.getEncoder()
                        .encodeToString((반응_하는_사람.getEmail() + ":" + 반응_하는_사람.getPassword()).getBytes()));
    ...
    void 게시글에_좋아요를_취소_한다() throws Exception {
        ...
        final MockHttpServletRequestBuilder 좋아요_취소_요청 = delete("/posts/" + 글.getId() + "/up")
                .header(HttpHeaders.AUTHORIZATION, "Basic "
                        + java.util.Base64.getEncoder()
                        .encodeToString((반응_하는_사람.getEmail() + ":" + 반응_하는_사람.getPassword()).getBytes()));
    ...
    void 게시글에_싫어요를_취소_한다() throws Exception {
        ...
        final MockHttpServletRequestBuilder 싫어요_취소_요청 = delete("/posts/" + 글.getId() + "/down")
                .header(HttpHeaders.AUTHORIZATION, "Basic "
                        + java.util.Base64.getEncoder()
                        .encodeToString((반응_하는_사람.getEmail() + ":" + 반응_하는_사람.getPassword()).getBytes()));
